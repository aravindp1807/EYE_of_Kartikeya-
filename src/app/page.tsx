...
const KartikeyaChat = dynamic(() => import('@/components/KartikeyaChat'), { ssr: false });
import KartikeyaHub from '@/components/KartikeyaHub';
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <AnimatePresence>
        {showSplash && (
          ...
          >
            ...
            {/* ── EYE OF KARTIKEYA title — letter-by-letter stagger ── */}
            <div className="flex flex-col items-center gap-1 mb-3 z-[2]">
              <div className="flex items-center gap-[3px]">
                {'EYE OF'.split('').map((letter, i) => (
                  <motion.span
                    key={`prefix-${i}`}
                    initial={{ opacity: 0, y: 20, filter: 'blur(8px)' }}
                    animate={{ opacity: 1, y: 0, filter: 'blur(0px)' }}
                    transition={{ delay: 0.3 + i * 0.06, duration: 0.5, ease: 'easeOut' }}
                    className="text-xl md:text-2xl font-bold tracking-[0.4em] font-mono"
                    style={{ color: 'var(--text-muted)', textShadow: 'none' }}
                  >
                    {letter === ' ' ? '\u00A0' : letter}
                  </motion.span>
                ))}
              ...
              <div className="flex items-center gap-[2px]">
                {'KARTIKEYA'.split('').map((letter, i) => (
                  <motion.span
                    ...
                    style={{ fontFamily: 'var(--font-title, serif)', color: 'var(--text-heading)', textShadow: '0 0 40px rgba(200,140,20,0.25), 0 0 80px rgba(180,20,20,0.1)' }}
                  >
                    {letter}
                  </motion.span>
                ))}
            ...
            <div className="overflow-hidden mb-8 z-[2]">
              ...
              >
                <p className="text-[10px] md:text-[11px] font-mono tracking-[0.5em] text-[var(--gold-primary)]" style={{ opacity: 0.85 }}>
                  SOVEREIGN INTELLIGENCE COMMAND
                </p>
              </motion.div>
            </div>
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '100px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          {/* Kartikeya Vel (Divine Spear) SVG */}
          <svg viewBox="0 0 100 140" className="w-8 h-10 md:w-10 md:h-12 shrink-0 transition-colors duration-500 drop-shadow-[0_0_10px_rgba(200,60,20,0.6)]" fill="none" xmlns="http://www.w3.org/2000/svg">
            {/* Spear tip */}
            <polygon points="50,2 58,28 50,22 42,28" fill="#C88C14" opacity="0.95"/>
            {/* Spear wings / Vel blades */}
            <polygon points="50,18 72,38 50,32 28,38" fill="#CC1A1A" opacity="0.85"/>
            {/* Central shaft */}
            <rect x="47" y="28" width="6" height="80" rx="3" fill="#C88C14" opacity="0.9"/>
            {/* Base crossguard */}
            <rect x="34" y="100" width="32" height="5" rx="2.5" fill="#C88C14" opacity="0.7"/>
            {/* Butt spike */}
            <polygon points="50,105 54,118 50,130 46,118" fill="#9B6A10" opacity="0.8"/>
            {/* Central eye (six-petal Shatkon center) */}
            <circle cx="50" cy="56" r="5" fill="#CC1A1A" opacity="0.9"/>
            <circle cx="50" cy="56" r="2.5" fill="#F5E8C8" opacity="0.95"/>
          </svg>
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-base md:text-lg font-bold tracking-[0.15em] text-[var(--text-heading)]" style={{ fontFamily: 'var(--font-title, serif)', textShadow: '0 0 20px rgba(200,140,20,0.3)' }}>EYE OF KARTIKEYA</h1>
            <span className="text-[7px] md:text-[8px] font-mono tracking-[0.2em] opacity-75 uppercase" style={{ color: 'var(--gold-primary)' }}>SOVEREIGN INTELLIGENCE COMMAND</span>
          </div>
        </div>
        <div className="flex items-center gap-3 mt-1.5 pl-[44px] min-w-0 pr-4">
          <span className="text-[5px] md:text-[6px] font-mono tracking-[0.2em] md:tracking-[0.3em] uppercase opacity-40 truncate" style={{ color: 'var(--text-muted)' }}>
            POWERED BY EYE OF KARTIKEYA INTELLIGENCE <span className="hidden md:inline">· C2 ENGINE: SOVEREIGN COMMAND CORE · SENSORS: ORBITAL LATTICE · NET: VEL NETWORK</span>
          </span>
        </div>
      </motion.div>
      ...
      {/* ── NEW KARTIKEYA RADIAL HUB ── */}
      {!isMobile && (
        <KartikeyaHub
          activePanel={showChat ? 'chat' : showIntel ? 'recon' : showMarkets ? 'markets' : showAlerts ? 'alerts' : showEntityGraph ? 'entityGraph' : null}
          onTogglePanel={(panel) => {
            if (panel === 'chat') { setShowChat(!showChat); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); setShowEntityGraph(false); }
            if (panel === 'recon') { setShowIntel(!showIntel); setShowChat(false); setShowMarkets(false); setShowAlerts(false); setShowEntityGraph(false); }
            if (panel === 'markets') { setShowMarkets(!showMarkets); setShowChat(false); setShowIntel(false); setShowAlerts(false); setShowEntityGraph(false); }
            if (panel === 'alerts') { setShowAlerts(!showAlerts); setShowChat(false); setShowIntel(false); setShowMarkets(false); setShowEntityGraph(false); }
            if (panel === 'entityGraph') { setShowEntityGraph(!showEntityGraph); setShowChat(false); setShowIntel(false); setShowMarkets(false); setShowAlerts(false); }
          }}
        />
      )}

      {/* ── PANELS TRIGEGERED BY HUB ── */}
      <AnimatePresence>
        {showChat && !isMobile && (
          <KartikeyaChat
            feedData={data}
        ...
        {showIntel && !isMobile && (
          <motion.div initial={{ opacity: 0, scale: 0.9, x: 20 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9, x: 20 }} className="absolute bottom-48 right-12 w-80 z-[250] pointer-events-auto">
            <OsintPanel theme={osirisTheme} setTheme={setOsirisTheme} onSweepVisualize={setSweepData} onScanGeolocate={(target, data) => {
              setScanTargets(prev => [{ id: target, timestamp: Date.now(), ...data }, ...prev.filter(t => t.id !== target)].slice(0, 10));
              setFlyToLocation({ lat: data.lat, lng: data.lng, ts: Date.now() });
            }} />
          </motion.div>
        )}

        {showMarkets && !isMobile && (
          <motion.div initial={{ opacity: 0, scale: 0.9, x: 20 }} animate={{ opacity: 1, scale: 1, x: 0 }} exit={{ opacity: 0, scale: 0.9, x: 20 }} className="absolute bottom-48 right-12 w-80 z-[250] pointer-events-auto">
